...
namespace consecution {
            ...
            value)
    {
        ...
        const Natural
            index
            = static_cast<Natural>(rank - sequence.source);
            ...
            to)
    {
        ...
        const Natural
            remaining
            = sequence.allotment - static_cast<Natural>(rank - sequence.source);
        while (direction.scale.order.equality.is_not_equal(apposition, to)) {
            if (Safety && !direction.traverses(space, apposition, 1))
                return false;
            direction.scale.traverse(space, apposition, 1);
            ...
            value)
    {
        ...
        const Natural
            index
            = static_cast<Natural>(position - sequence.source);
        if (!Move(position, position + 1, sequence.allotment - index) && Safety)
            return false;
        Refer(position).to = value;
        sequence.allotment++;
            ...
            to)
    {
        ...
        const Natural
            remaining
            = sequence.allotment - static_cast<Natural>(first - sequence.source);
        while (direction.scale.order.equality.is_not_equal(apposition, to)) {
            if (Safety && !direction.traverses(space, apposition, 1))
                return false;
            direction.scale.traverse(space, apposition, 1);
